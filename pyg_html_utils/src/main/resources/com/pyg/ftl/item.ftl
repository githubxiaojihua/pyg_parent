...
<body ng-app="pyg" ng-controller="itemController" ng-init="num=1;loadSku()">
   <#include "head.ftl">
   <#-- 处理图片列表-->
							...
							<#list specificationList as specification>
                                <dl>
								 ...
								 <#list specification.attributeValue as item>
								     <dd><a ng-click="selectSpecification('${specification.attributeName}','${item}')" class="{{isSelected('${specification.attributeName}','${item}')?'selected':''}}" >${item}</a></dd>
 								 </#list>
                                </dl>
							</#list>
